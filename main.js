...
// Application State
const appState = {
  currentScreen: 'lock', // lock, intro, gifts, collage, blessing, our_story, cake, bmw, video, apology, ending
...
function renderApp() {
  ...
  else if (appState.currentScreen === 'our_story') content = renderOurStoryScreen();
  ...
  if (appState.currentScreen === 'our_story') startOurStorySequence();
  ...
  return `
    <div class="screen blessing-screen flex-center" id="blessing-stage">
      ...
      <div class="thatha-scene" id="thatha-scene">
        ...
        <img src="./images/thatha-blessing.jpeg" alt="Thatha's Blessing" class="thatha-figure" id="thatha-figure" />
      </div>
      ...
      <!-- Continue Button -->
      <button class="bl-continue hidden" id="bl-continue" onclick="window.navigate('our_story')">
        Continue 🎂
      </button>
    </div>
  `;
...
function startOurStorySequence() {
  ...
  const pc = document.getElementById('our-story-particles');
  if (pc) {
    const pInterval = setInterval(() => {
      if (!document.getElementById('our-story-stage')) { clearInterval(pInterval); return; }
      const p = document.createElement('div');
